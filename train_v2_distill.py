...
def main(args):
    ...
    if args.prune_ratio:
        ...
        img = torch.from_numpy(img).unsqueeze(0).half().cuda()

        # org_macs, org_nparams, org_flops = get_model_infos(backbone, img, "arcface_model")
        ...
        pruner = tp.pruner.GroupNormPruner(
            ...
            iterative_steps=1,
            pruning_ratio=args.prune_ratio, #0.25 是 4M左右
        ...
        if args.prune_load:
            ...
            weight = torch.load(args.prune_load, map_location=torch.device("cpu"))
            backbone.load_state_dict(weight, strict=False)


        # prune_macs, prune_nparams, prune_flops = get_model_infos(backbone, img, "arcface_model-prune", [org_macs, org_nparams, org_flops])
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels) in enumerate(train_loader):
            ...
            if teacher_model is not None and distill_criterion is not None:
                with torch.no_grad():
                    teacher_emb = teacher_model(img)
                loss, distill_loss_val = distill_criterion(local_embeddings, teacher_emb, ce_loss)
            else:
                loss = ce_loss
            ...
            if cfg.fp16:
                ...
                if global_step % cfg.gradient_acc == 0:
                    amp.unscale_(opt)
                    torch.nn.utils.clip_grad_norm_(list(backbone.parameters()) + list(module_partial_fc.parameters()), 5)
                    amp.step(opt)
                    amp.update()
                    opt.zero_grad()
            else:
            ...
            with torch.no_grad():
                if wandb_logger:
                    ...
                    if teacher_model is not None:
                        log_dict['Loss/Distill Loss'] = distill_loss_val
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prune_ratio", type=float, default=0, help="float vaule for prunning")
    parser.add_argument("--prune_load", type=str, help="load weight path str for loading pruned model")
