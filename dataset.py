    ...
    def __init__(self, root_dir, local_rank):
        ...
        class SimulateInfrared(object):
            ...
            def __call__(self, img):
                ...
                # 1) 更接近监控红外风格的加权灰度（R/G 主导，B 较弱）
                ir = 0.6 * img_np[:, :, 0] + 0.35 * img_np[:, :, 1] + 0.05 * img_np[:, :, 2]
                # 2) gamma < 1 提升中高亮区域（皮肤更亮）
                ir = ir / 255.0
                gamma = 0.75
                ir = np.power(ir, gamma)
                # 3) 提升整体对比度，让背景更暗、人脸更亮
                mean = ir.mean()
                contrast = 1.5
                ir = (ir - mean) * contrast + mean
                ir = np.clip(ir * 255.0, 0, 255).astype(np.uint8)
                # 4) 保持 3 通道（兼容后续 Normalize）
                ir_3 = np.stack([ir, ir, ir], axis=-1)
                from PIL import Image
                return Image.fromarray(ir_3)

        self.transform = transforms.Compose(
            [
                transforms.ToPILImage(),
                transforms.RandomHorizontalFlip(),
                SimulateInfrared(prob=0.6),
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),
            ]
        )
